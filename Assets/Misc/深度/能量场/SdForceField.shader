Shader "MyShader/ForceField"
{
    ...
    SubShader
    {
        ...
        Tags
        {
            "RenderType"="Transparent"  //配合使用替代渲染的方法(Camera.SetReplacementShader)
            "Queue"="Transparent"
        }
        GrabPass
        {
        ...
        Pass
        {
            ...
            {
                //获取已有的深度信息,此时的深度图里没有力场的信息,Queue = Transparent 的不会被存进深度图,没有ShadowCaster通道，或者没有单独画进深度图的不会进入深度图
                //判断相交
                ...
                float rim = (1 - abs(dot(normalize(i.normal) , normalize(i.viewDir)))) * _RimStrength;
